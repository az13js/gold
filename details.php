...
if (!is_dir('details') && false === mkdir('details')) {
    echo '创建目录details失败' . PHP_EOL;
    die();
}
...
$urls = explode(PHP_EOL, file_get_contents('links.txt'));

try {
    $tool = new PageDownloadTool($h);
    foreach ($urls as $k => $url) {
        if (empty($url) || is_file("details/$k.html")) {
            continue;
        }
        echo $url . PHP_EOL;
        $result = $tool->get($url, $base);
        file_put_contents("details/$k.html", $result);
    }
} catch (Exception $e) {
